...
#include "rom_ekf_robot/cylinder_detect.h"
...
/// \return (tf2d::Vector2D) x,y coordinates of the point
tf2d::Vector2D polar2cart(double r, double theta)
{
  return tf2d::Vector2D(r*std::cos(theta), r*std::sin(theta));
}
...
void callback_robotScan(sensor_msgs::LaserScan::ConstPtr data)
{
  ...
  std::vector<tf2d::Vector2D> temp_points; // Temporary cluster of points
  std::vector<std::vector<tf2d::Vector2D>> buf_points_list; // list of all point clusters
  std::vector<std::vector<tf2d::Vector2D>> points_list; // list of all point clusters
  ...
  rom_ekf_robot::MapData cluster_data;
...
int main(int argc, char** argv)
{
  ...
  pub_cmd = n.advertise<rom_ekf_robot::MapData>("landmark_data", 1);
  ...
  distance_threshold = 0.075;  // 0.075
  radius_threshold = 0.09; // 0.07
  frame_id = "base_scan";
  //frame_id = "top_lidar";
