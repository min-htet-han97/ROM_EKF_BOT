...
{
	ros::init(argc, argv, "ukf_slam_node");
	ros::NodeHandle nh;
	ros::NodeHandle pnh("~");
	...
	Ukf_Slam ukf_slam(nh, num_landmarks, wheel_radius, wheel_separation, map_frame_, odom_frame_, base_frame_, right_wheel_joint_, left_wheel_joint_);
	ros::spin();
	return 0;
}